# MUIC Class Committee 678 Website

A mobile-first website with the main goal of event coordination.

Home page:

![HomePage](gitimg/page0.jpg)

Event board:

![EventPage](gitimg/page_board.jpg)

## Highlights
 - Used cloudflare zero-trust solution to ensure secure HTTPS connection.
 - Announced "Rubnong" groups number to freshmen using student ID based authentication.
 - Paired seniors with freshmen as part of the "Secret Hug" event, aimed to help new students navigate their first year at MUIC.
 - Coordinated "Gift Exchange" events. Ensuring that the event runs smoothly using automated table partitioning algorithm and image upload system for gift identification, with email and student ID based authentication.
 - Event board for displaying upcoming events, along with relevant links such as sign-ups, informations, etc.
